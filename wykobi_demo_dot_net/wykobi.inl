...
namespace wykobi
{
	...
	inline bool intersect(const ray<T, 2> & ray, const rectangle<T> & rectangle)
	{
		...
		if (not_equal(ray.direction.x, T(0.0)))
		{
			...
			{
				if ((t = (rectangle[1].x - ray.origin.x) * recip_dirx) < tmin)
				{
					return false;
				} tmax = min(t, tmax);
				if ((t = (rectangle[0].x - ray.origin.x) * recip_dirx) > tmax)
				{
					return false;
				} tmin = max(t, tmin);
			}
			...
			{
				if ((t = (rectangle[0].x - ray.origin.x) * recip_dirx) < tmin)
				{
					return false;
				} tmax = min(t, tmax);
				if ((t = (rectangle[1].x - ray.origin.x) * recip_dirx) > tmax)
				{
					return false;
				} tmin = max(t, tmin);
			}
		...
		if (not_equal(ray.direction.y, T(0.0)))
		{
			...
			{
				if ((t = (rectangle[1].y - ray.origin.y) * recip_diry) < tmin)
				{
					return false;
				} tmax = min(t, tmax);
				if ((t = (rectangle[0].y - ray.origin.y) * recip_diry) > tmax)
				{
					return false;
				} tmin = max(t, tmin);
			}
			...
			{
				if ((t = (rectangle[0].y - ray.origin.y) * recip_diry) < tmin)
				{
					return false;
				} tmax = min(t, tmax);
				if ((t = (rectangle[1].y - ray.origin.y) * recip_diry) > tmax)
				{
					return false;
				} tmin = max(t, tmin);
			}
	...
	inline bool intersect(const ray<T, 3> & ray, const box<T, 3> & box)
	{
		...
		if (not_equal(ray.direction.x, T(0.0)))
		{
			...
			if (ray.direction.x > T(0.0))
			{
				if ((t = (box[1].x - ray.origin.x) * recip_dirx) < tmin)
				{
					return false;
				} tmax = min(t, tmax);
				if ((t = (box[0].x - ray.origin.x) * recip_dirx) > tmax)
				{
					return false;
				} tmin = max(t, tmin);
			}
			...
			{
				if ((t = (box[0].x - ray.origin.x) * recip_dirx) < tmin)
				{
					return false;
				} tmax = min(t, tmax);
				if ((t = (box[1].x - ray.origin.x) * recip_dirx) > tmax)
				{
					return false;
				} tmin = max(t, tmin);
			}
		...
		if (not_equal(ray.direction.y, T(0.0)))
		{
			...
			if (ray.direction.y > T(0.0))
			{
				if ((t = (box[1].y - ray.origin.y) * recip_diry) < tmin)
				{
					return false;
				} tmax = min(t, tmax);
				if ((t = (box[0].y - ray.origin.y) * recip_diry) > tmax)
				{
					return false;
				} tmin = max(t, tmin);
			}
			...
			{
				if ((t = (box[0].y - ray.origin.y) * recip_diry) < tmin)
				{
					return false;
				} tmax = min(t, tmax);
				if ((t = (box[1].y - ray.origin.y) * recip_diry) > tmax)
				{
					return false;
				} tmin = max(t, tmin);
			}
		...
		if (not_equal(ray.direction.z, T(0.0)))
		{
			...
			{
				if ((t = (box[1].z - ray.origin.z) * recip_dirz) < tmin)
				{
					return false;
				} tmax = min(t, tmax);
				if ((t = (box[0].z - ray.origin.z) * recip_dirz) > tmax)
				{
					return false;
				} tmin = max(t, tmin);
			}
			...
			{
				if ((t = (box[0].z - ray.origin.z) * recip_dirz) < tmin)
				{
					return false;
				} tmax = min(t, tmax);
				if ((t = (box[1].z - ray.origin.z) * recip_dirz) > tmax)
				{
					return false;
				} tmin = max(t, tmin);
			}
